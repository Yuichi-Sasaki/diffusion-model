...
class DiffusionModel(object):
    ...
    def prepare_alphas(self):
        ...
        betas_t_div_sqrt_one_minus_alphas_cumprod = (
            betas / sqrt_one_minus_alphas_cumprod
        )
        sqrt_posterior_variance = np.sqrt(self.posterior_variance)

        vars = {}
        vars["betas"] = betas
        vars["alphas"] = alphas
        vars["alphas_cumprod"] = alphas_cumprod
        vars["alphas_cumprod_prev"] = alphas_cumprod_prev
        vars["sqrt_recip_alphas"] = sqrt_recip_alphas
        vars["sqrt_alphas_cumprod"] = sqrt_alphas_cumprod
        vars["sqrt_one_miums_alphas_cumprod"] = sqrt_one_minus_alphas_cumprod
        vars["posterior_variance"] = posterior_variance
        vars[
            "betas_t_div_sqrt_one_minus_alphas_cumprod"
        ] = betas_t_div_sqrt_one_minus_alphas_cumprod
        vars["sqrt_posterior_variance"] = sqrt_posterior_variance

        import csv

        fout = open("alphas.csv", "w")
            ...
            for i in range(5):
                ...
                imgs_pick = [
                    ...
                    for i in np.linspace(
                        0, self.timesteps - 1, 5, dtype=np.int32
                    ).tolist()
                ]
